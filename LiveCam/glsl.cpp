...
written in the Graphics Shading language. Vertex Shaders, Geometry Shaders and Fragment shaders
...
// GL ERROR CHECK
   int CheckGLError(char *file, int line)
   {
	   ...
	   //Temp workaround to solve bug when hang on exit -- Vladislav Khorev vladislav.khorev@fishrungames.com
...
//----------------------------------------------------------------------------- 
   bool InitGraphicsExtensions(void)
   {
      ...
      cout << "Graphics Vendor: " << (char*) glGetString(GL_VENDOR) << "\n";
      cout << "Graphics Renderer: " << (char*) glGetString(GL_RENDERER) << "\n";
      cout << "Graphics Version: " << (char*) glGetString(GL_VERSION) << "\n\n";
      //cout << "Graphics Extensions:\n" << (char*) glGetString(GL_EXTENSIONS) << "\n\n";
   ...
   bool HasGLSLSupport(void)
   {
      ...
      if (!extensions_init) InitGraphicsExtensions();  // extensions were not yet initialized!!

      
      if (GLEW_VERSION_2_0)
      {
         cout << "Graphics 2.0 (or higher) is available!" << endl;
      }
      else if (GLEW_VERSION_1_5)
      {
         cout << "Graphics 1.5 core functions are available" << endl;
      }
      else if (GLEW_VERSION_1_4)
      {
         cout << "Graphics 1.4 core functions are available" << endl;
      }
      else if (GLEW_VERSION_1_3)
      {
         cout << "Graphics 1.3 core functions are available" << endl;
      }
      else if (GLEW_VERSION_1_2)
      {
         cout << "Graphics 1.2 core functions are available" << endl;
      }
      ...
      if (useGLSL)
      {
          cout << "[OK] Graphics Shading Language is available!\n\n";
      }
      else
      {
          cout << "[FAILED] Graphics Shading Language is not available...\n\n";
      }   
   ...
   bool HasGraphics2Support(void)
   {
      if (!extensions_init) InitGraphicsExtensions();
...
glShader::glShader()
{
  ...
  if (!useGLSL)
  {
     cout << "**ERROR: Graphics Shading Language is NOT available!" << endl;
  }
  else
  {
...
//----------------------------------------------------------------------------- 
int glShader::getVetrexShaderAttribLocation(char* attribName)
{
	GLint ret = glGetAttribLocation(ProgramObject, attribName);
	//if (ShaderList.size() > 0)
	//	ret = ShaderList[0]->getAttribLocation(attribName);
	return ret;
}
...
glShader* glShaderManager::loadfromFile(const char* vertexFile, const char* fragmentFile)
{
   ...
   if (vertexFile != 0)
   {
	   ...
	   {
		   qDebug() << "error: can't load vertex shader: " << vertexFile;
		   delete o;
  ...
  // Load fragment program
   if (fragmentFile != 0)
   {
	   ...
	   {
		   qDebug() << "error: can't load fragment shader: " << fragmentFile;
		   delete o;
		   delete tVertexShader;
		   delete tFragmentShader;
		   return 0;
