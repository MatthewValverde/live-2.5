...
#include "fx/FxWidgetFacePinch.h"
#include <Common/Mesh3DMaker.h>
...
FxPinch::FxPinch()
{
	...
	auto faceDistortion = make_shared<GraphicsFaceDistortionModel>();
...
void FxPinch::transformMesh(cv::Mat frame, std::vector<FXModel>& faces, Mesh3D *model)
{
...
void FxPinch::transformMeshOne(cv::Mat frame, const FXModel& fxModel, Mesh2D& imgMesh)
{
	const std::array<float, 66 * 2>&  pts = fxModel.pts;

	FxWidgetFacePinch pinchMaker(frame.size(), imgMesh);
