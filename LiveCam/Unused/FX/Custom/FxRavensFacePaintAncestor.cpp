...
static const cv::Point2f topPointTexCoord = { 0.5f, 0.2f };
...
{
	{
		{ 8, 57, 7, 58, 6, 59, 5, 48, 4 },
		{ 41, 36, 1, 2, 3, 4, 48, 49, 31, 40 },
		{ 1, 0, 36, 17, 37, 18, 38, 20, 39, -1 },
		{ 39,-1, 27, 28, 29, 30, 31, 40 },
		{ 31, 30, 33, 51, 50, 49 }
	},
	{
		{ 8, 57, 9, 56, 10, 55, 11, 54, 12 },
		{ 46, 45, 15, 14, 13, 12, 54, 53, 35, 47 },
		{ 15, 16, 45, 26, 44, 25, 43, 23, 42, -1 },
		{ 42, -1, 27, 28, 29, 30, 35, 47 },
		{ 35, 30, 33, 51, 52, 53 }
	}
};
...
void RavensHalfFacePaint::draw(FXModel& face, RavensHalfFaceParams &params, std::shared_ptr<cwc::glShader> shader)
{
	...
	{
		throw std::exception("Not implemented, use FxWidgetFacePaint");
	}
	else
	{
		{
...
void RavensFacePaintModel::onInputFrameResized()
{
	...
	for (auto &face : halfFacePaint.facePointsSmoothers)
	{
		...
		{
			smoother.SMOOTH_MODIFIER = 1.0 / ValueSmoother::SMOOTH_FACTOR_COORDS * 1280.0 / Resolutions::INPUT_ACTUAL_WIDTH;
		}
	}
}
...
void RavensFacePaintModel::applySuit(boost::property_tree::ptree& suit, size_t targetIndex, TCommonRenderParams &commonRenderParams,
...
void RavensFacePaintModel::transform(FXModel& face, ExternalRenderParams &externalRenderParams)
{
	...
	double width = widthRawSmooth / face.frameWidth;
