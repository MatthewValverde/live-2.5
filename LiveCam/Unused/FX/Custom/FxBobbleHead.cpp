...
std::array<float, facePointCount * 2> calculatePointPosArr(const FXModel& face);

FxBobbleHead::FxBobbleHead()
{
	...
	headModel->modelPath = "./assets/fx/reds/bobble_head/models/Bobblehead_Head_Object_UVMap_002.obj";
	headModel->name = "head";
	...
	hairModel->name = "hair";
	hairModel->modelShift = { 0, 0.26f, 0 };
	hairModel->modelScale = 1.85f;
...
void FxBobbleHead::load()
{
	FX::load();

	std::string hairTexture = "./assets/fx/reds/bobble_head/textures/hair.png";

	std::string normalTexture = "./assets/fx/reds/bobble_head/textures/hair_normal.png";

	std::string headTexture = "./assets/fx/reds/bobble_head/textures/BobbleHead_UVMap_Template.jpg";
...
void FxBobbleHead::transform(FXModel& face, ExternalRenderParams &externalRenderParams)
{
...
void FxBobbleHead::draw(FXModel& face, ExternalRenderParams &externalRenderParams)
{
	...
	std::array<float, facePointCount * 2> pointPosArr = calculatePointPosArr(face);
...
void FxBobbleHead::animateNextStep(size_t i)
{
	if (animationDirection[i]) {

		animationCounter[i]++;

		if (animationCounter[i] == maxMovement)
		{
...
void FxBobbleHead::calcNewFaceCenterAndStd()
{

	
}

cv::Point2f FxBobbleHead::calcFaceStd(const std::array<float, 66 * 2>& pts)
{
	float stdFaceX = 0;
	float stdFaceY = 0;
