...
{
	if (!description->enabled || !description->particleShader)
	{
		return;
	}
...
void Particle::transform(ParticleUpdateParams& params)
{
	...
	{
		effect_gravity_shift = params.effect_scaling.block<3, 3>(0, 0).matrix() * (description->gravity * lifeTime * lifeTime / 2);
	...
	{
		global_gravity_shift = coords_system->getOuterRotation() * params.globalGravity * lifeTime * lifeTime / 2;
	}
	else
	{
