...
FaceContours CVImageUtil::extractFaceContourInFrame(TrackingTarget& face)
{
	FaceContours result;

	cv::Size2i frameSize = { face.frameWidth, face.frameHeight };
	cv::Point2i leftTop = { face.frameWidth, face.frameHeight };
...
CVImageUtil::FrameLightParams CVImageUtil::getFrameLightParams(cv::Mat& frame)
{
	...
	if (del > 0)
	{
		...
		result.brightness = STD_FRAME_BRIGHTNESS + (result.brightness - STD_FRAME_BRIGHTNESS) * 3;
	}
	else
	{
		result.light = { 1, 1, 1 };
...
CVImageUtil::FaceLightParams CVImageUtil::getFaceLightParams(cv::Mat& frame, FaceContours& contours, TrackingTarget& face)
{
