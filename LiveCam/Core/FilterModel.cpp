...
FilterModel::FilterModel(FilterUiModel* externalInfo)
{
...
void FilterModel::load()
{
	FX::load();
}

void FilterModel::transform(TrackingTarget& target, ExternalRenderParams &externalRenderParams)
{
	FX::transform(target, externalRenderParams);
	...
	for (auto model : models)
	{
		...
		for (auto &param : model3D->objectRenderParams[target.pointId])
		{
