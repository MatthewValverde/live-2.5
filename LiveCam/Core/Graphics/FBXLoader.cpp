...
void FBXLoader::CollectMeshes(shape_fbx *shape, FbxScene * pScene)
{
	...
	if (lRootNode) {
		...
		shape->mesh = lRootNode->GetMesh();
	}
}

bool FBXLoader::LoadFile(std::vector<shape_fbx> *shapes)
{
	bool lResult = false;
...
void FBXLoader::DrawMesh(FbxNode* pNode, FbxTime& pTime, FbxAnimLayer* pAnimLayer,FbxAMatrix& pGlobalPosition, FbxPose* pPose)
{
	...
	{
		// Graphics driver is too lower and use Immediate Mode
