...
namespace gl {
    ...
    ,immediate_must_fill(true)
  {
    ...
    // ORTHO MATRIX
	ortho_matrix = Eigen::Matrix4f::Zero();
    ...
    texture_mesh->push_back(VertexPT(cv::Point2f(0.0f,   th),  cv::Vec2f(0.0f, 1.0f)) );
    texture_mesh->push_back(VertexPT(cv::Point2f(tw,     th),  cv::Vec2f(1.0f, 1.0f)) );
    texture_mesh->push_back(VertexPT(cv::Point2f(tw,   0.0f),  cv::Vec2f(1.0f, 0.0f)) );
    texture_mesh->push_back(VertexPT(cv::Point2f(0.0f,   th),  cv::Vec2f(0.0f, 1.0f)) );
    texture_mesh->push_back(VertexPT(cv::Point2f(tw,   0.0f),  cv::Vec2f(1.0f, 0.0f)) );
    texture_mesh->push_back(VertexPT(cv::Point2f(0.0f, 0.0f),  cv::Vec2f(0.0f, 0.0f)) );
    texture_mesh->update();
    ...
    for(int i = 0; i < circle_resolution; ++i) {
		cv::Point3f aPoint(((float)cos(i * rad)), (sin(i * rad)), 0.0f);
		cv::Vec4f aColor(1.0f, 1.0f, 1.0f);
      VertexCP v(aColor, aPoint);
  ...
  void Drawer::setupShader(Shader& shader) {
    ...
    shader.setProjectionMatrix(ortho_matrix.data());
    shader.setViewMatrix(identity.data());
    shader.setModelMatrix(identity.data());
  }
  ...
  void Drawer::drawTexture(Texture& tex, float x, float y, float w, float h) {
    ...
    shader_tex.use();
    shader_tex.setModelMatrix(texture_mesh->model_matrix.data());
  ...
  void Drawer::drawArrays(Mesh<VertexP>& mesh, GLenum mode, GLint first, GLsizei count) {
    ...
    mesh.bind();
    mesh.update();
    shader_p.setModelMatrix(mesh.mm().data());
  ...
  void Drawer::drawArrays(Mesh<VertexPT>& mesh, GLenum mode, GLint first, GLsizei count) {
    ...
    mesh.bind();
    mesh.update();
    shader_pt.setModelMatrix(mesh.mm().data());
    glDrawArrays(mode, first, count);
    mesh.unbind();
    shader_pt.unuse();
  }
  ...
  void Drawer::drawArrays(Mesh<VertexNP>& mesh, GLenum mode, GLint first, GLsizei count) {
    ...
    mesh.bind();
    mesh.update();
    shader_np.setModelMatrix(mesh.mm().data());
    glDrawArrays(mode, first, count);
    mesh.unbind();
    shader_np.unuse();
  }
  ...
  void Drawer::color(cv::Vec4f color) {
    immediate_color = color;
  }

  void Drawer::vertex(cv::Point3f position) {
  ...
  void Drawer::drawCircle(float x, float y, float radius) {
    ...
    for(size_t i = 0; i < immediate_circle.size(); ++i) {
      ...
      cv::Vec3f pos(x + v.pos.x * radius, y + v.pos.y * radius, 0.0f);
      vertex(pos);
    }
    end();
  }
  ...
  void Drawer::drawRectangle(float x, float y, float w, float h) {
	  if (immediate_must_fill) {
		  begin(GL_TRIANGLES);
		  vertex(cv::Vec3f(x, y + h, 0));     // bottom left
		  vertex(cv::Vec3f(x + w, y + h, 0)); // bottom right
		  vertex(cv::Vec3f(x + w, y, 0));;    // top right 

		  vertex(cv::Vec3f(x, y + h, 0));     // bottom left
		  vertex(cv::Vec3f(x + w, y, 0));;    // top right 
		  vertex(cv::Vec3f(x, y, 0));         // top left
		  end();
	  }
	  else {
		  begin(GL_LINE_LOOP);
...
#define GLR_PRINT_INFO_INTEGERV(field) {         \
  ...
  void glr_print_info() {
    ...
    LC_VERBOSE("GL_MAJOR_VERSION: %d, GL_MINOR_VERSION: %d", major, minor);
  ...
  const float* glr_get_orthographic_matrix() {
    assert(glr_context);
    return glr_context->ortho_matrix.data();
  }
  ...
  void glr_color(float r, float g, float b, float a) {
    glr_color(cv::Vec4f(r,g,b,a));
  }

  void glr_vertex(cv::Vec3f position) {
    assert(glr_context);
