...
namespace cwc
{
   ...
   //! Shader Object holds the Shader Source, the Graphics "Shader Object" and provides some methods to load shaders.
   /*!
...
//-----------------------------------------------------------------------------
   ...
   public:
      ...
      //!< Returns the Graphics Program Object (only needed if you want to control everything yourself) \return The Graphics Program Object
      ...
      void       begin();                           //!< use Shader. Graphics calls will go through vertex, geometry and/or fragment shaders.
      void       end();                             //!< Stop using this shader. Graphics calls will go through regular pipeline.
      ...
      \warning NVidia implementation is different than the GLSL standard: GLSL attempts to eliminate aliasing of vertex attributes but this is integral to NVIDIA’s hardware approach and necessary for maintaining compatibility with existing Graphics applications that NVIDIA customers rely on. NVIDIA’s GLSL implementation therefore does not allow built-in vertex attributes to collide with a generic vertex attributes that is assigned to a particular vertex  attribute index with glBindAttribLocation. For example, you should not use gl_Normal (a built-in vertex attribute) and also use glBindAttribLocation to bind a generic vertex attribute named "whatever" to vertex attribute index 2 because gl_Normal aliases to index 2.
      \verbatim
      gl_Vertex                0
      gl_Normal                2
      gl_Color                 3
      ...
      */
      ...
      //! Disable this Shader:
	   ...
	   int      getVetrexShaderAttribLocation(char* attribName);
   protected:
...
//-----------------------------------------------------------------------------
   ...
   static float lc_random(float max) {
	   return max * rand() / (RAND_MAX + 1.0f);
   }

   static float lc_random(float x, float y) {
	   float high = 0;
	   float low = 0;
	   float result = 0;
...
//-----------------------------------------------------------------------------
// Global functions to initialize Graphics Extensions and check for GLSL and 
// Graphics2. Also functions to check if Shader Model 4 is available and if
// Geometry Shaders are supported.
bool GLSLAPI InitGraphicsExtensions(void); //!< Initialize Graphics Extensions (using glew) \ingroup GLSL
bool GLSLAPI HasGLSLSupport(void);       //!< Returns true if Graphics Shading Language is supported. (This function will return a GLSL version number in a future release) \ingroup GLSL  
bool GLSLAPI HasGraphics2Support(void);    //!< Returns true if Graphics 2.0 is supported. This function is deprecated and shouldn't be used anymore. \ingroup GLSL \deprecated
...
#define initGLExtensions InitGraphicsExtensions
#define checkGLSL HasGLSLSupport
#define checkGL2  HasGraphics2Support
//----------------------------------------------------------------------------
