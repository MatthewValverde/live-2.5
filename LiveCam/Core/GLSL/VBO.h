#ifndef LIVECAM_OPENGL_VBO_H
#define LIVECAM_OPENGL_VBO_H

#include <GLSL/GL.h>
#include <Utils/Log.h>
#include <GLSL/Vertices.h>
...
namespace gl {
  ...
  template<class T>
    ...
    if(id) {
      LC_ERROR(ERR_GL_VBO_ALREADY_SETUP);
      return false;
    }
    ...
    if(!id) {
      LC_ERROR(ERR_GL_VBO_CANNOT_CREATE);
      return false;
    }
