...
namespace gl {
	...
	// VERTEX_P
  // ---------------------------------------------------
	static const char* GL_VS_P = GL_PARSE_SHADER(150,
		in vec4 a_pos;
	uniform mat4 u_pm;
	uniform mat4 u_vm;
	uniform mat4 u_mm;
	...
	static const char* GL_FS_P = GL_PARSE_SHADER(150,
		out vec4 frag_color;
	void main() {
	...
	// VERTEX_CP
	// ---------------------------------------------------
	static const char* GL_VS_CP = GL_PARSE_SHADER(150,
		in vec4 a_pos;
	in vec4 a_col;
	uniform mat4 u_pm;
	uniform mat4 u_vm;
	...
	static const char* GL_FS_CP = GL_PARSE_SHADER(150,
		in vec4 v_col;
	out vec4 frag_color;
	void main() {
		frag_color = v_col;
	...
	// VERTEX_PT
	// ---------------------------------------------------
	static const char* GL_VS_PT = GL_PARSE_SHADER(150,
		in vec4 a_pos;
	in vec2 a_tex;
	uniform mat4 u_pm;
	uniform mat4 u_vm;
	...
	static const char* GL_FS_PT = GL_PARSE_SHADER(150,
	...
	// VERTEX_NP
	// ---------------------------------------------------
	static const char* GL_VS_NP = GL_PARSE_SHADER(150,
		in vec4 a_pos;
	in vec3 a_norm;
	uniform mat4 u_pm;
	uniform mat4 u_vm;
	...
	static const char* GL_FS_NP = GL_PARSE_SHADER(150,
		in vec3 v_norm;
	out vec4 frag_color;
	void main() {
  ...
  public:
    ...
    void color(cv::Vec4f color);                                                            /* immediate mode: set the color for the next vertex */
    void vertex(cv::Point3f position);                                                        /* immediate mode: end the current sequence of vertices */
  ...
  public:
    ...
    Eigen::Matrix4f ortho_matrix;                                                      /* orthographic projection */
    ...
    cv::Vec4f immediate_color;                                                              /* immediate mode: currently set color; is used for the vertex that is added when you call `Drawer::vertex()` */
 ...
 // ---------------------------------------------------
  ...
  void glr_color(cv::Vec4f color);                                                          /* immediate mode: set the color for the next vertex */
  ...
  void glr_vertex(cv::Vec3f position);                                                      /* immediate mode: end the current sequence of vertices */
