...
namespace gl {
  ...
  bool Shader::load(std::string vs, std::string fs, bool datapath) {
    ...
    std::string vss = lc_get_file_contents(vs, datapath);
    if(!vss.size()) {
      LC_ERROR(ERR_GL_SH_LOAD_FILE, vs.c_str());
      return false;
    }

    std::string fss = lc_get_file_contents(fs, datapath);
    if(!fss.size()) {
      LC_ERROR(ERR_GL_SH_LOAD_FILE, fs.c_str());
      return false;
    }
  ...
  bool Shader::create(std::string vs, std::string fs) {
    ...
    if(prog) {
      LC_ERROR(ERR_GL_SH_ALREADY_CREATED);
      return false;
    }
    ...
    if(!vert_id) {
      LC_ERROR(ERR_GL_SH_CANNOT_CREATE_VERT);
    ...
    if(!frag_id) {
      LC_ERROR(ERR_GL_SH_CANNOT_CREATE_FRAG);
      glDeleteShader(vert_id);
      glDeleteShader(frag_id);
      return false;
    }
    ...
    if(!prog) {
      LC_ERROR(ERR_GL_SH_CANNOT_CREATE_PROG);
      glDeleteShader(vert_id);
      glDeleteShader(frag_id);
      glDeleteProgram(prog);
      return false;
  ...
  bool Shader::link() {
    if(!vert_id || !frag_id) {
      LC_ERROR(ERR_GL_SH_NO_VERT_OR_FRAG);
      return false;
    }

    if(!prog) {
      LC_ERROR(ERR_GL_SH_PROG_NOT_CREATED);
      return false;
    }
  ...
  void Shader::activeTexture(Texture& tex, GLenum unit) {
    ...
    if(unit == GL_TEXTURE0 && u_tex0 == -1) {
      LC_ERROR(ERR_GL_SH_NO_UNI_TEX, 0);
      return;
    }
    else if(unit == GL_TEXTURE1 && u_tex1 == -1) {
      LC_ERROR(ERR_GL_SH_NO_UNI_TEX, 1);
      return;
    }
    else if(unit == GL_TEXTURE2 && u_tex2 == -1) {
      LC_ERROR(ERR_GL_SH_NO_UNI_TEX, 2);
      return;
    }
    else if(unit == GL_TEXTURE3 && u_tex3 == -1) {
      LC_ERROR(ERR_GL_SH_NO_UNI_TEX, 3);
      return;
    }
  ...
  void Shader::activeTexture(Texture& tex, GLenum unit, GLint uniform) {
    ...
    switch(unit) {
      ...
      default: {
        LC_ERROR(ERR_GL_SH_UNSUPPORTED_TEXUNIT);
        break;
      }
    }
  }
  ...
  void Shader::print() {
    LC_VERBOSE("Shader.prog: %d", prog);
    LC_VERBOSE("Shader.u_pm: %d", u_pm);
    LC_VERBOSE("Shader.u_vm: %d", u_vm);
    LC_VERBOSE("Shader.u_mm: %d", u_mm);
    LC_VERBOSE("Shader.u_tex0: %d", u_tex0);
    LC_VERBOSE("Shader.u_tex1: %d", u_tex1);
    LC_VERBOSE("Shader.u_tex2: %d", u_tex2);
    LC_VERBOSE("Shader.u_tex3: %d", u_tex3);
  }
};
