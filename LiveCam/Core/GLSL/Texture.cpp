...
namespace gl {
  ...
  bool Texture::load(std::string filepath, bool datapath) {
    ...
    if(!img.load(filepath, datapath)) {
      LC_ERROR(ERR_GL_CANNOT_LOAD_FILE, filepath.c_str());
      return false;
    }
  ...
  bool Texture::setPixels(Image& img) {
    ...
    if(img.getPixelFormat() == LC_FMT_RGB24) {
      ...
      LC_ERROR(ERR_GL_FORMAT_NOT_SUPPORTED);
      return false;
    }
                          ...
                          GLenum type)
  {
    ...
    if(type != GL_UNSIGNED_BYTE) {
      LC_ERROR(ERR_GL_UNSUPPORTED_TYPE);
      return false;
    }
    ...
    if(!id) {
      ...
      if(!id) {
        LC_ERROR(ERR_GL_CANNOT_CREATE_TEX);
        return false;
      }
    }

    if(is_allocated && ((w != width) || (h != height)) ) {
      LC_ERROR(ERR_GL_DIFFERENT_SIZE);
      return false;
    }
  ...
  void Texture::print() {
    LC_VERBOSE("Texture.target: %s", targetToString(target).c_str());
    LC_VERBOSE("Texture.internal_format: %s", formatToString(internal_format).c_str());
    LC_VERBOSE("Texture.format: %s", formatToString(format).c_str());
    LC_VERBOSE("Texture.type: %s", typeToString(type).c_str());
    LC_VERBOSE("Texture.width: %d", width);
    LC_VERBOSE("Texture.height: %d", height);
  }
