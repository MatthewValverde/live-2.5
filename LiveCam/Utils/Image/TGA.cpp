...
bool TGA::save(string filepath, bool datapath) { 
  LC_ERROR("We need to fix this function!!");
...
bool TGA::load(string filepath, bool datapath) {
  ...
  color_type = header[2];
	...
	num_bytes = (lc_uint32)width * height * (bits_per_pixel / 8);
	if(pixels != NULL) {
		delete[] pixels;
	}
...
void TGA::print() {
	LC_VERBOSE("TGA.header: %s", header);
	LC_VERBOSE("TGA.width: %d", width);
	LC_VERBOSE("TGA.height: %d", height);
	LC_VERBOSE("TGA.bits_per_pixel: %d", bits_per_pixel)
	LC_VERBOSE("TGA.id: %d", id);
	LC_VERBOSE("TGA.num bytes: %zu", num_bytes);	
  LC_VERBOSE("TGA.color_type: %s", colorTypeToString(color_type).c_str());
}
