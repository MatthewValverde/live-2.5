...
bool Image::copyPixels(unsigned char* pix, unsigned int w, unsigned int h, unsigned format) {
  ...
  if(!pixels) {
    LC_ERROR(ERR_IMG_CANNOT_ALLOCATE);
    return false;
  }
...
bool Image::load(std::string filename, bool datapath) {
  ...
  if(ext == "png") {
    return lc_load_png(*this, filename, datapath);
  }
  else if(ext == "jpg") {
    return lc_load_jpg(*this, filename, datapath);
  }
  else if(ext == "tga") {
    return lc_load_tga(*this, filename, datapath);
  }
  else {
    LC_ERROR(ERR_IMG_UNSUPPORTED_EXT);
    return false;
  }
...
bool Image::save(std::string filename, bool datapath) {
  ...
  if(ext == "png") {
    return lc_save_png(*this, filename, datapath);
  }
  return false;
}
...
bool lc_load_png(Image& img, std::string filename, bool datapath) {
  ...
  switch(png.getColorType()) {
    case PNG_COLOR_TYPE_GRAY: {
      img.setPixelFormat(LC_FMT_GRAY8);
      break;
    }
    case PNG_COLOR_TYPE_RGB: {
      img.setPixelFormat(LC_FMT_RGB24);
      break;
    }
    case PNG_COLOR_TYPE_RGB_ALPHA: {
      img.setPixelFormat(LC_FMT_RGBA32);
      break;
    }
    default: {
      LC_ERROR(PNG_ERR_UNSUPPORTED_FORMAT);
      return false;
    }
  }
...
bool lc_load_jpg(Image& img, std::string filename, bool datapath) {
  ...
  if(jpg.getNumChannels() == 3) {
    ...
    LC_ERROR(ERR_IMG_UNSUPPORTED_FORMAT);
    return false;
  }
...
bool lc_load_tga(Image& img, std::string filename, bool datapath) {
...
bool lc_save_png(Image& img, std::string filename, bool datapath) {
  
  PNG png;

  png_uint_32 type;

  if(img.getPixelFormat() == LC_FMT_GRAY8) {
    type = PNG_COLOR_TYPE_GRAY;
  }
  else if(img.getPixelFormat() == LC_FMT_RGB24) {
    type = PNG_COLOR_TYPE_RGB;
  }
  else if(img.getPixelFormat() == LC_FMT_RGBA32) {
