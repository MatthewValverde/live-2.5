...
bool PNG::load(std::string filename, bool datapath) {
...
#ifdef LIVECAM_DEBUG
  LC_VERBOSE("The png library results in wierd behaviour when compiling on windows in debug mode");
#endif  
  ...
  if(pixels) {
    LC_ERROR(PNG_ERR_ALREADY_LOADED);
    return false;
  }
  
  if(datapath) {
    filename = lc_to_data_path(filename);
  }

  FILE* fp;
  fopen_s(&fp, filename.c_str(), "rb");
  if(!fp) {
    LC_ERROR(PNG_ERR_LOAD_NO_FILE, filename.c_str());
    fp = NULL;
    return false;
  }
  ...
  if(!png_ptr) {
    LC_ERROR(PNG_ERR_CREATE_READ_ST);
    fclose(fp);
    fp = NULL;
    return false;
  }
  ...
  if(!info_ptr) {
    ...
    LC_ERROR(PNG_ERR_CREATE_INFO_PTR);
    fclose(fp);
    fp = NULL;
    return false;
  }
...
bool PNG::save(std::string filename, bool datapath) {
  ...
  if(!width || !height) {
    LC_ERROR(PNG_ERR_SIZE, width, height);
    return false;
  }

  if(!stride) {
    LC_ERROR(PNG_ERR_STRIDE, stride);
    return false;
  }

  if(!pixels) {
    LC_ERROR(PNG_ERR_PIXELS);
    return false;
  }

  if(!bit_depth) {
    LC_ERROR(PNG_ERR_BITDEPTH, bit_depth);
    return false;
  }
  ...
  if(!fp) {
    LC_ERROR(PNG_ERR_SAVE_NO_FILE, filename.c_str());
    fp = NULL;
    return false;
  }
  ...
  if(png_ptr == NULL) {
    LC_ERROR(PNG_ERR_CREATE_WRITE_ST);
    fclose(fp);
    fp = NULL;
    return false;
  }
...
void PNG::print() {
  LC_VERBOSE(PNG_V_WIDTH, width);
  LC_VERBOSE(PNG_V_HEIGHT, height);
  LC_VERBOSE(PNG_V_BITDEPTH, bit_depth);
  LC_VERBOSE(PNG_V_NCHANNELS, num_channels);
  LC_VERBOSE(PNG_V_COLORTYPE, colorTypeToString(color_type).c_str());
  LC_VERBOSE(PNG_V_NUMBYTES, num_bytes);
}
