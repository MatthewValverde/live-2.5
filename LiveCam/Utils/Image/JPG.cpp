...
bool JPG::load(std::string filename, bool datapath) {
  ...
  if(fp == NULL) {
    LC_ERROR(ERR_JPG_FILE_NOT_OPENED, filename.c_str());
    return false;
  }
...
bool JPG::load(unsigned char* compressed, size_t nbytes) {
  ...
  if(rc != 1) {
    LC_ERROR("Error while reading the jpeg header");
    return false;
  }
  ...
  if(!width || !height) {
    LC_ERROR("Read incorrect jpg size: %d x %d", width, height);
    jpeg_finish_decompress(&cinfo);
    jpeg_destroy_decompress(&cinfo);
    return false;
  }
...
bool JPG::setPixels(unsigned char* pix, int w, int h, J_COLOR_SPACE type) {
  ...
  if(!pix) {
    LC_ERROR("Invalid pixels given");
    return false;
  }

  if(!w) {
    LC_ERROR("Invalid width given");
    return false;
  }

  if(!h) {
    LC_ERROR("invalid height given");
    return false;
  }
  ...
  switch(color_space) {
    ...
    default: {
      LC_ERROR("Unhandled color space: %d", type);
      return false;
    }
  }
  ...
  pixels = new unsigned char[num_bytes];
  if(!pixels) {
    LC_ERROR("Cannot allocate %ld bytes for the jpg", num_bytes);
    return false;
  }
...
bool JPG::save(std::string filename, bool datapath) {
  ...
  if(!width || !height) {
    LC_ERROR("Invalid width or height: %d x %d", width, height);
    return false;
  }
  
  if(!stride) {
    LC_ERROR("Invalid stride: %d", stride);
    return false;
  }
  
  if(!pixels) {
    LC_ERROR("No pixels set to save");
    return false;
  }

  if(!bit_depth) {
    LC_ERROR("Invalid bit_depth: %d", bit_depth);
    return false;
  }
  ...
  if(!fp) {
    LC_ERROR("Cannot open the file: `%s`", filename.c_str());
    fp = NULL;
    return false;
  }
...
void JPG::print() {
  LC_VERBOSE("JPG.width: %d", width);
  LC_VERBOSE("JPG.height: %d", height);
  LC_VERBOSE("JPG.bit_depth: %d", bit_depth);
  LC_VERBOSE("JPG.num_channels: %d", num_channels);
}
