...
void EditorWindow::resetEditableFilter()
{
	...
	editableFilter =std::make_shared<FX>();
	...
	auto text1 = ui.filterName_edit->text();

	auto text =text1.toStdString();
...
std::shared_ptr<FX> EditorWindow::getEditableFilter()
{
...
void EditorWindow::readFilterProperties(std::shared_ptr<FX> filter, FilterUiModel& externalInfo)
{
	resetEditableFilter();
	...
	for (auto model : filter->models)
	{
		...
		if (type == FxWidget2D::TYPE_NAME)
		{
			auto tab = createEmojiModelTab();
			tab->readEmojiModelProperties(filter.get(), (FxWidget2D*)model.get(), filter->filterModules[index].suits[index],
				filter->resourcesRoot);
		}
		else if (type == FxWidget3D::TYPE_NAME)
		{
			auto tab = createModel3DTab(false);
			tab->read3DModelProperties(filter.get(), (FxWidget3D*)model.get(), (FxWidget3D*)editableFilter->models.back().get(),
...
EmojiModelTab* EditorWindow::createEmojiModelTab()
{
	...
	auto emoji =std::make_shared<FxWidget2D>();
	...
	auto list = liveCamWindow->ui.moduleList->findChildren<QPushButton*>();
	...
	{
		liveCamWindow->ui.horizontalLayout_moduleList->itemAt(visible)->widget()->setVisible(true);
	}
...
Model3DTab* EditorWindow::createModel3DTab(bool lipsJoint)
{
	...
	std::shared_ptr<FxWidget3D> model = lipsJoint ?std::make_shared<LipsJoint3DModel>() :std::make_shared<FxWidget3D>();
...
void EditorWindow::saveFilter(QString& filterFolder)
{
	...
	liveCamWindow->ui.showFiltersButton->setEnabled(true);
...
void EditorWindow::on_import_button_clicked()
{
	...
	auto filter =std::make_shared<FilterModel>(&externalInfo);
